...
class MatchThreeGame:
    def __init__(self, width: int = 8, height: int = 8, colors: int = 5):
        self.width = width
        self.height = height
        self.colors = colors
        self.board = []
        ...
        for y in range(self.height):
            for x in range(self.width):
                if (x, y) not in visited and self.board[y][x] != 0:
                    # ОШИБКА 1: Ищем во всех направлениях, включая диагонали
    ...
    def fill_board_buggy(self) -> None:
        ...
        for x in range(self.width):
            for y in range(self.height):
                if self.board[y][x] == 0:
                    # ОШИБКА 2: Просто заполняем пустоты на месте, без падения
                    self.board[y][x] = random.randint(1, self.colors)
                    changes_made = True
        
        # Задержка после добавления новых ячеек
        if changes_made:
            print("Добавление новых ячеек (ОШИБКА: без падения!)...")
            self.print_board()
            time.sleep(0.5)
